...
document.addEventListener('DOMContentLoaded', () => {
  ...
  let lang = localStorage.getItem('keyboardLang')
    ? Number(localStorage.getItem('keyboardLang'))
    : 0;
  ...
  keys.forEach((key) => {
    ...
    keyElement.addEventListener('click', (event) => {
      ...
      if (key === '◄') {
        ...
        if (prevLineIndex !== -1) {
          ...
          input.selectionStart = prevLineStart
            + Math.min(currLineOffset, input.value.length - prevLineStart - 1);
          input.selectionEnd = input.selectionStart;
        }
      } else if (key === '▼') {
        const lines = textareaValue.split('\n');
        ...
        if (nextLineStart !== 0 && nextLineStart <= textareaValue.length) {
          const currLineStart = input.value.lastIndexOf('\n', input.selectionStart + 1) + 1;
          const currLineOffset = input.selectionStart - currLineStart;
          input.selectionStart = nextLineStart
            + Math.min(currLineOffset, textareaValue.length - nextLineStart);
          input.selectionEnd = input.selectionStart;
        }
      }
    });
    keyElement.addEventListener('mousedown', (event) => {
      ...
      if (key === 'Backspace') {
        ...
        keys.forEach((key, i) => {
          const keyElement = document.querySelectorAll('.key')[i];
          keyElement.innerHTML = upperCase
            ? key[langIndex].toUpperCase()
            : key[langIndex].toLowerCase();
        ...
        input.value = `${input.value.slice(0, start)}  ${input.value.slice(
          end,
        )}`;
  ...
  document.addEventListener('keydown', (event) => {
    ...
    if (event.key === 'CapsLock') {
      ...
      if (keyValue === 'CapsLock') {
        ...
        const letter = capsLockOn || shiftPressed
  ...
  function updateKeys() {
    ...
    keyElements.forEach((keyElement, i) => {
      ...
      if (capsPressed) {
        keyElement.innerHTML = shiftPressed
          ? key.toLowerCase()
          : key.toUpperCase();
      } else {
        keyElement.innerHTML = shiftPressed
